...
const UNISWAP_V3_FACTORIES = new Set([
    ...
    '0x1128f23d0bc0a8396e9fbc3c0c68f5ea228b8256', // Unknown V3 fork (active)
])
...
async function getPoolTokens(pool: string, cachedRPC: CachedRPC): Promise<{ token0: string; token1: string } | null> {
    ...
    try {
        ...
        if (!UNISWAP_V3_FACTORIES.has(factory.toLowerCase())) return null
        ...
        for (const [pool, poolLogs] of logsByPool) {
            ...
            for (const log of poolLogs) {
                ...
                swaps.push({
                    ...
                    poolType: uniswapV3.poolType,
                    providerName: uniswapV3.name,
                })
            }
        }
