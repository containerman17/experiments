...
export default function ICTTHomes() {
    ...
    const homesByChain = useMemo(() => {
        const grouped = new Map<string, IcttContractHome[]>()
        homes.forEach(home => {
            const key = `${home.chainName}-${home.evmChainId}`
            if (!grouped.has(key)) {
                grouped.set(key, [])
            }
            grouped.get(key)!.push(home)
        })
    ...
    return (
        <div className="py-8">
            ...
            {homesByChain.map(([chainKey, chainHomes]) => {
                const [chainName, evmChainId] = chainKey.split('-')
                return (
                    <div key={chainKey} className="mb-12">
                                                                ...
                                                                <td className="px-4 py-3 text-sm">
                                                                    <div>
                                                                        <div className="font-medium text-gray-900">
                                                                            {remoteChain?.chainName || 'Unknown Chain'}
                                                                        </div>
                                                                        <div className="text-xs text-gray-500">
                                                                            ID: {remoteChain?.evmChainId || 'N/A'}
                                                                        </div>
                                                                    </div>
                                                                </td>
                                                                <td className="px-4 py-3 text-sm">
