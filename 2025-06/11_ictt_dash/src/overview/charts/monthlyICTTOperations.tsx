...
import { getApiGlobalOverviewMonthlyIcttOperationsByChainCompact } from "../../client/sdk.gen"
...
const description = "ICTT (Interchain Token Transfer) messages. Every message is recorded twice (once for each direction)."

export default function MonthlyICTTOperations() {
    const { data, isError, isLoading } = useQuery<CompactData>({
        queryKey: ['monthlyICTTOperationsByChainCompact'],
        queryFn: async () => {
            const response = await getApiGlobalOverviewMonthlyIcttOperationsByChainCompact();

            if (!response.data) {
                throw new Error('Failed to fetch monthly ICTT operations data');
            }
    ...
    if (isLoading) {
        return (
            <div className="bg-white border border-gray-200 rounded-xl p-6">
                <div className="mb-4">
                    <h3 className="text-lg font-semibold text-gray-900">Monthly ICTT Operations - All Indexed L1s</h3>
    ...
    // Calculate total operations for display
    const totalOperations = data.chains.reduce((sum, chain) =>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6">
            <div className="mb-4">
                ...
                <p className="text-sm text-gray-500 mt-1">
                    {totalOperations.toLocaleString()} total operations across {activeChainsOnly.length} active chains
                </p>
            </div>
            <div className="h-96">
