...
**BUG: wrong state root at block 867,931 (first mismatch).** Trie encoding confirmed correct — geth's StackTrie and our HashBuilder produce identical correct root at block 867,930. The issue is the EVM EXECUTION of block 867,931 (WAVAX withdraw). The flat state after executing this block differs from the canonical chain. isMultiCoin flag is NOT the cause (no account has Extra set). Root computation (both full-scan and incremental) is correct. The execution itself writes wrong state. The `isMultiCoin` extra flag on accounts was not stored in our flat state encoding. WAVAX `withdraw()` calls the NativeAssetCall precompile which sets `isMultiCoin=true` on accounts. Our 104-byte account encoding didn't include this flag, so the RLP hash was wrong for any account with multi-coin balances. Fix: expanded account encoding to 105 bytes, `AccountLeafSource` now reads the flag and encodes correct RLP extra field (`0x01` for true, `0x80` for false).
