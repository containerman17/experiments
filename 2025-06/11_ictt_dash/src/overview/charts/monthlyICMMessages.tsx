...
import { getApiGlobalOverviewMonthlyIcmByDirectionByChainCompact } from "../../client/sdk.gen"

interface CompactData {
    dates: string[]; // e.g. ["2024-01", "2024-02", ...]
    chains: Array<{
        evmChainId: number;
        name: string;
        values: number[];
...
const generateChainColors = (numChains: number, direction: 'incoming' | 'outgoing'): string[] => {
    const colors: string[] = [];
    const hueStep = 360 / numChains;
    // Use different starting hues for incoming (blue) vs outgoing (green)
    const startHue = direction === 'incoming' ? 200 : 120;

    for (let i = 0; i < numChains; i++) {
        const hue = (i * hueStep + startHue) % 360;
...
interface MonthlyICMMessagesProps {
    direction: 'incoming' | 'outgoing';
}

export default function MonthlyICMMessages({ direction }: MonthlyICMMessagesProps) {

    const { data, isError, isLoading } = useQuery<CompactData>({
        queryKey: ['monthlyICMMessagesByChainCompact', direction],
        queryFn: async () => {
            const response = await getApiGlobalOverviewMonthlyIcmByDirectionByChainCompact({
                path: {
                    direction
                }
            });
            ...
            if (!response.data) {
                throw new Error('Failed to fetch monthly ICM messages data');
            }
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6">
                <div className="mb-4">
                    <h3 className="text-lg font-semibold text-gray-900">
                        Monthly {direction === 'incoming' ? 'Incoming' : 'Outgoing'} ICM Messages - All Indexed L1s
                    </h3>
                    <p className="text-sm text-gray-600">
                        {direction === 'outgoing'
                            ? 'Messages sent from each chain to other chains'
                            : 'Messages received by each chain from other chains'
                        } via Interchain Messaging (ICM/Teleporter) protocol
                    </p>
                </div>
    ...
    const colors = generateChainColors(data.chains.length, direction);

    const activeChainsOnly = data.chains.filter(chain =>
        chain.values.some(val => val > 0)
    );

    // Calculate total messages for display
    const totalMessages = data.chains.reduce((sum, chain) =>
        sum + chain.values.reduce((chainSum, val) => chainSum + val, 0), 0
    );
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6">
            <div className="mb-4">
                ...
                <p className="text-sm text-gray-500 mt-1">
                    {totalMessages.toLocaleString()} total {direction} messages across {activeChainsOnly.length} active chains
                </p>
            </div>
            <div className="h-96">
